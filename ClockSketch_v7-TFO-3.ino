/////////4/6个数字的钟,3个状态灯+每段三个灯88:88:88,共130个LED灯
/* -[ClockSketch v7.4]----------------------------------------------------------------------------------------
...
#ifdef USEWIFI
  const bool useWPS = true;          
  const char* wifiSSID = "BH4SKM";
  const char* wifiPWD = "18556772981";
#endif
...
uint8_t upperLimitLDR = 180;                     
uint8_t lowerLimitLDR = 50;                       
uint8_t minBrightness = 30;                       
const bool nightMode = false; 
const uint8_t nightColor[2] = { 0, 70 };                     
float factorLDR = 1.0;                            
const bool dbgLDR = false;                       
#ifdef NODEMCU
  uint8_t pinLDR = 1;                             
#else
  uint8_t pinLDR = 0;                            
#endif
uint8_t intervalLDR = 75;                         
uint16_t avgLDR = 0;                              
uint16_t lastAvgLDR = 0;                          
/* End autoBrightness config/parameters ---------------------------------------------------------------- */


#define SKETCHNAME "ClockSketch v7.4"
#define CLOCKNAME "VX BH4SKM"


/* Start button config/pins----------------------------------------------------------------------------- */
#ifdef NODEMCU
  const uint8_t buttonA = 13;                                    // 1 pin to d7 / GPIO_13
  const uint8_t buttonB = 14;                                    // 1 pin to d5 / GPIO_14
#else
  const uint8_t buttonA = 3;                                     //  1 pin to d3
  const uint8_t buttonB = 4;                                     //  1 pin to d4
#endif
...
const uint8_t brightnessLevels[3] {100, 130, 220};                // 0 - 255, brightness Levels (min, med, max) - index (0-2) will be saved to eeprom
...
#define LEDSTUFF
  ...
  #define LED_PWR_LIMIT 1000                                      // 500mA - Power limit in mA (voltage is set in setup() to 5v) 
  #define LED_DIGITS 6                                           // 4 or 6 digits, HH:MM or HH:MM:SS
  #define LED_COUNT 132                                          // Total number of leds, 103 on Retro 7 Segment Clock v3 - The Final One(s) - 3 LEDs/segment
  #if ( LED_DIGITS == 6 )
    #define LED_COUNT 133                                        // leds on the 6 digit version
  #endif
...
uint8_t digitsLAM[6] = { 1, 1, 1, 1, 1, 1 };
  ...
  /* digit position 4 */
 ,{ 94, 96 },                   // top, a      !! do not remove the "," at the start of this line !!
  { 91, 93 },                   // top right, b
  { 103, 105 },                   // bottom right, c
  { 106, 108 },                   // bottom, d
  { 109, 111 },                   // bottom left, e
  { 97, 99 },                   // top left, f
  { 100, 102 },                   // center, g
  /* digit position 5 */
  { 115, 117 },                   // top, a
  { 112, 114 },                   // top right, b
  { 124, 126 },                   // bottom right, c
  { 127, 129 },                   // bottom, d
  { 130, 132 },                   // bottom left, e
  { 118, 120 },                   // top left, f
  { 121, 123 }                    // center, g
...
#if ( LED_DIGITS == 4 )
  const uint16_t upperDots[2] PROGMEM = {  45,  46 };              // leds inside the upper dots (right on L7-QBE)
 // const uint16_t lowerDots[2] PROGMEM = {  89,  90 };              // leds inside the lower dots (left on L7-QBE)
#endif

#if ( LED_DIGITS == 6 )
  const uint16_t upperDots[4] PROGMEM = {  45,  89 };     // all the leds inside the upper dots (bogus values on some models which don't support 6 digits)
  const uint16_t lowerDots[4] PROGMEM = {  46,  90 };     // all the leds inside the lower dots (bogus values on some models which don't support 6 digits)
#endif
...
/* This sets time and date (if AUTODST is defined) on the clock/rtc */
  ...
  setupTime.Year = 23;                                                                      // current year - 2000 (2023 - 2000 = 23)
    ...
    uint8_t y, m, d;
    y = getUserInput(12, 20, 23, 99);                                                       // show Y + blank, get value from 21 - 99 into y
    setupTime.Year = y;                                                             // 2 digit year + 30 (epoch), so we get offset from 1970
...
/* This will show two symbols on HH and allow to enter a 2 digit value using the buttons
  ...
  while ( lastInput != 2 ) {
    ...
    FastLED.clear();
    showDigit(sym1, digitPositions[3]);
    showDigit(sym2, digitPositions[2]);
    showDigit(currentVal / 10, digitPositions[1]);
    showDigit(currentVal % 10, digitPositions[0]);
    if ( millis() % 30 == 0 ) {
      colorizeOutput(colorMode);
      FastLED.show();
      }
...
void displayTime(time_t t) {
  ...
  if ( displayMode == 0 ) {
    ...
    if ( hourFormat12(t) < 10 ) {
      ...
      showDigit(hourFormat12(t) / 10, digitPositions[0]);   //以下几个序号要改过来,现在测试第一段闪烁
    }
